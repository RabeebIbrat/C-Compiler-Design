...
public:
    ...
    deque<string> *return_var_list = new deque<string>;  ///for assembly code
    deque<int> *return_size_list = new deque<int>;  ///for assembly code
        ...
        if(var_type.compare("VOID") == 0) {
            ...
            while(!var_list->empty()) {
                ...
                if(!table->insert(name,var_type,size)) {
                    ...
                    return_size_list->push_back(size);
                }
            }
            var_type = "";
        }
...
public:
    ...
    string asmCode = "";
    string asmCodePush = "";  ///required in function definition
    string asmCodePop = "";  ///required in function definition
    string varName = "<none>";
    string varIndex = "<none>";

    void printAsm(string fileName, int line_count) {  ///for debug purposes
        FILE *asmFile;
        mkdir("asmFolder");
        asmFile = fopen( ("asmFolder/" + fileName + ".asm").c_str() , "a");
        fprintf(asmFile, "<LINE: %d>\n\n%s\n------------------\n", line_count, asmCode.c_str());
    }

    void printAsm(string fileName, int line_count, string output) {  ///for debug purposes
        FILE *asmFile;
        mkdir("asmFolder");
        asmFile = fopen( ("asmFolder/" + fileName + ".asm").c_str() , "a");
        fprintf(asmFile, "<LINE: %d>\n\n%s\n------------------\n", line_count, output.c_str());
    }

    void printFinalAsm(string fileName) {
        FILE *asmFile;
        asmFile = fopen( (fileName + ".asm").c_str() , "w");
        fprintf(asmFile, asmCode.c_str());
    }
